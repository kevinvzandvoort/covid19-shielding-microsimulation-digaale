analysis_name = "risk_bytime_full2"
...
target_prior_immunity_values = 0 #scenario_list[, unique(prior_immunity)]
...
singleFunction = function(iteration_data){
  ...
  model_out = seq_len(length(scens)) %>% lapply(function(s, scens){
    ...
    if(shielded_prop == 1)
      prevalence_by_time_by_age = rbind(prevalence_by_time_by_age[age < 6], prevalence_by_time_by_age[age >= 6 & shielded_status == 1])
...
#sbatch_command = prepareSlurmBatchRuns(analysis_name, singleFunction, iterations, main_folder, out_folder, packages_func, populations, overwrite = T)
result = combineSlurmResults(analysis_name, iterations)

risk_bytime_colours = c(
  #"Unmitigated: individuals aged 60+y" = "#FFB81C",
  "Individuals aged <60y who are not shielded" = "#0D5257",
  "Individuals aged 60+y who are not shielded" = "#00BF6F",
  "Individuals aged 60+y who are shielded" = "#00AEC7")
...
plotdat_sub = plotdat %>%
  .[type == c("all", "match")[1] & iv_shielded_unshielded_contacts %in% c(0.2, 0.6) & iv_shielded_prop %in% c(0, 0.8)]
...
plotdat_sub = plotdat_sub[iv_shielded_prop == 0, -c("iv_shielded_prop", "iv_shielded_gz_size", "shielded_status",
  ...
  .[, clrname := factor(paste0(scen_type, age, ": ", factor(shielded_status, 0:1, c("unshielded", "shielded"))),
                        c("value_unmit>=60: unshielded", "value_mit>=60: unshielded",
                          "value_mit>=60: shielded", "value_mit<60: unshielded"),
                        c("Unmitigated: individuals aged 60+y",
                          "Individuals aged 60+y who are not shielded",
                          "Individuals aged 60+y who are shielded",
                          "Individuals aged <60y who are not shielded"))] %>%
  .[time <= 100 & clrname %in% c("Individuals aged <60y who are not shielded",
                                 "Individuals aged 60+y who are not shielded",
                                 "Individuals aged 60+y who are shielded")]
...
ggplot(mapping=aes(x=time, y=median, ymin=low95, ymax=high95, colour=clrname, fill=clrname, group=clrname))+
  ...
  facet_grid(iv_shielded_unshielded_contacts ~ iv_shielded_gz_size, labeller = labeller(
    iv_shielded_unshielded_contacts = function(x) paste0((1-as.numeric(x))*100,"%")))+
  geom_text(data = data.table(iv_shielded_unshielded_contacts = 0.2, iv_shielded_gz_size = 2, label = "Shielded residence size", x = 0,
            ...
            show.legend = FALSE)+
  geom_text(data = data.table(iv_shielded_unshielded_contacts = 0.2, iv_shielded_gz_size = 16, label = "Reduction in contacts between\nshielded and unshielded",
                              x = 130, y = -0.175), aes(x=x, y=y, label=label, ymin=NULL, ymax=NULL, colour=NULL,
                                                   linetype=NULL, fill=NULL, group=NULL), hjust = 0.5, vjust = 0,
  ...
  #geom_line(data = plotdat_sub[scen_type == "value_unmit" & age == ">=60" & shielded_status == 0], size=1, alpha=0.75)+
  geom_line(data = plotdat_sub[scen_type == "value_mit"], aes(linetype = age), size=1, alpha=0.75)+
  guides(color = guide_legend(nrow = 2, byrow = FALSE), fill = guide_legend(nrow = 2, byrow = FALSE),
         linetype = guide_none())+
  ...
  theme_bw()+
  theme(plot.margin = unit(c(1.1, 1.8, 3, 0.2), "lines"), axis.title.y = element_text(size = 8),
...
plotdat_sub = plotdat_sub[iv_shielded_prop == 0, -c("iv_shielded_prop", "iv_shielded_gz_size", "shielded_status",
  ...
  .[, clrname := factor(paste0(scen_type, age, ": ", factor(shielded_status, 0:1, c("unshielded", "shielded"))),
                        ...
                        c("Unmitigated: individuals aged 60+y",
                          ...
                          "Individuals aged 60+y who are shielded",
                          "Individuals aged <60y who are not shielded"))] %>%
  .[time <= 150 & iv_shielded_gz_size %in% c(2,16) & clrname %in% c("Individuals aged <60y who are not shielded",
                                                                    "Individuals aged 60+y who are not shielded",
                                                                    "Individuals aged 60+y who are shielded")]
...
plot_r0_sens_a = ggplot(mapping=aes(x=time, y=median, ymin=low95, ymax=high95, colour=clrname, fill=clrname, group=clrname))+
  ...
  #geom_ribbon(data = plotdat_sub_a[scen_type == "value_unmit" & age == ">=60" & shielded_status == 0], alpha=0.2, colour=NA)+
...
plot_r0_sens_b = ggplot(mapping=aes(x=time, y=median, ymin=low95, ymax=high95, colour=clrname, fill=clrname, group=clrname))+
  ...
  geom_text(data = data.table(target_R0 = 1.5, iv_shielded_gz_size = 16, label = "R[0]",
                              x = 184, y = 1), aes(x=x, y=y, label=label, ymin=NULL, ymax=NULL, colour=NULL,
                                                   linetype=NULL, fill=NULL, group=NULL), hjust = 0, vjust = 0,
            angle = -90, size = 8/(72.27/25.4), show.legend = FALSE, parse = TRUE)+
  #geom_ribbon(data = plotdat_sub_b[scen_type == "value_unmit" & age == ">=60" & shielded_status == 0], alpha=0.2, colour=NA)+
...
result[time == 200] %>% merge(scenario_list, by="scen") %>% .[iv_shielded_prop == 0 & type == "all" & age == ">=60"] %>%
  dcast(...~variable, value.var="value") %>% .[, sprintf("%s%% (%s - %s)", round(median*100), round(low95*100), round(high95*100)), by=c("target_R0", "prior_immunity")]


model_out[time == 200 & type == "all" & age == ">=60"] %>% merge(scenario_list, by="scen") %>% dcast(...~variable, value.var = "value") %>%
  .[, sprintf("%s%% (%s - %s)", round(median*100), round(low95*100), round(high95*100)), by=c("target_R0", "prior_immunity")]
